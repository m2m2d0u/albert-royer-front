...
export const deleteUser = async (id) => HTTP.delete(`user/${id}`, {headers: {Authorization: `Bearer ${authHeader().access_token}`}})
...
export const fetchOrSearchJobs = (data) => HTTP.get(`job?${data}`)

export const fetchById = (id) => HTTP.get(`job/${id}`, {headers: {Authorization: `Bearer ${authHeader().access_token}`}})

export const updateJob = (id, data) => HTTP.put(`job/${id}`, data, {headers: {Authorization: `Bearer ${authHeader().access_token}`}})

export const addJob = (data) => HTTP.post(`job`, data, {headers: {Authorization: `Bearer ${authHeader().access_token}`}})

export const fetchByTest = (id) => HTTP.get(`job/${id}/test`)
